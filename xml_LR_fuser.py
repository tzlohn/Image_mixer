...
working_folder = filedialog.askdirectory(title = "selecting the folders containing images of both sides")
...
for n in range(page_num_right):
    ...
    shutil.move(name,dest_folder[0])
...
for n in range(page_num_left):
    ...
    TFF.imwrite(name, an_image, bigtiff = True)
    shutil.move(name,dest_folder[1])
...
if len(all_right_positions) > 1:
    offset_V = abs(all_right_positions[1] - all_right_positions[0])
elif len(all_left_positions) >1:
    offset_V = abs(all_left_positions[1] - all_left_positions[0])
else:
    offset_V = 0 
